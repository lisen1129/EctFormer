...
Hnet = Ectformer(in_channel=36, out_channel=12)
Rnet = Ectformer(in_channel=12, out_channel=24)
...
with torch.no_grad():
    ...
    for  img in tqdm(datasets.DIV2K_multi_val_loader, total=len(datasets.DIV2K_multi_val_loader)):
        ...
        secret_input = torch.cat([secret_input1, secret_input2], dim=1)
        ...
        secret_rev = torch.cat((secret_rev1, secret_rev2), dim=1)
        ...
        #
        # print(i, "PSNR:", np.mean(psnr_c), np.mean(psnr_1s), np.mean(psnr_2s))
        # print(i, "SSIM:", np.mean(ssim_c), np.mean(ssim_1s), np.mean(ssim_2s))
        ...
        torchvision.utils.save_image(cover, c.IMAGE2_PATH_cover + '%.5d.png' % i)
        torchvision.utils.save_image(secret1, c.IMAGE2_PATH_secret1 + '%.5d.png' % i)
        torchvision.utils.save_image(secret2, c.IMAGE2_PATH_secret2 + '%.5d.png' % i)
        torchvision.utils.save_image(steg_img, c.IMAGE2_PATH_stego + '%.5d.png' % i)
        torchvision.utils.save_image(secret_rev1, c.IMAGE2_PATH_secret_rev1 + '%.5d.png' % i)
        torchvision.utils.save_image(secret_rev2, c.IMAGE2_PATH_secret_rev2 + '%.5d.png' % i)
        i = i + 1


    print("PSNR:", np.mean(psnr_c_list), np.mean(psnr_1s_list), np.mean(psnr_2s_list))
    print("SSIM:", np.mean(ssim_c_list), np.mean(ssim_1s_list), np.mean(ssim_2s_list))
