...
#
#     return peak_signal_noise_ratio(origin, pred, data_range=255)
def computePSNR(origin, pred):
    origin = np.array(origin)
    origin = origin.astype(np.float32)
    pred = np.array(pred)
...
with torch.no_grad():
    ...
    for img in tqdm(datasets.DIV2K_multi_val_loader, total=len(datasets.DIV2K_multi_val_loader)):
        ...
        psnr_4s = computePSNR(deal_cpu(secret4), deal_cpu(secret_rev4))
        ssim_4s = computeSSIM(deal_cpu(secret4), deal_cpu(secret_rev4))
        ...
        psnr_4s_list.append(psnr_4s)
        ssim_4s_list.append(ssim_4s)

        if i > 1000:            break
        # print(psnr_c,psnr_s)

        torchvision.utils.save_image(cover, c.IMAGE4_PATH_cover + '%.5d.png' % i)
        torchvision.utils.save_image(secret1, c.IMAGE4_PATH_secret1 + '%.5d.png' % i)
        torchvision.utils.save_image(secret2, c.IMAGE4_PATH_secret2 + '%.5d.png' % i)
        torchvision.utils.save_image(secret3, c.IMAGE4_PATH_secret3 + '%.5d.png' % i)
        torchvision.utils.save_image(secret4, c.IMAGE4_PATH_secret4 + '%.5d.png' % i)
        torchvision.utils.save_image(steg_img, c.IMAGE4_PATH_stego + '%.5d.png' % i)
        torchvision.utils.save_image(secret_rev1, c.IMAGE4_PATH_secret_rev1 + '%.5d.png' % i)
        torchvision.utils.save_image(secret_rev2, c.IMAGE4_PATH_secret_rev2 + '%.5d.png' % i)
        torchvision.utils.save_image(secret_rev3, c.IMAGE4_PATH_secret_rev3 + '%.5d.png' % i)
        torchvision.utils.save_image(secret_rev4, c.IMAGE4_PATH_secret_rev4 + '%.5d.png' % i)
        i = i + 1

    print("PSNR:", np.mean(psnr_c_list), np.mean(psnr_1s_list), np.mean(psnr_2s_list), np.mean(psnr_3s_list), np.mean(psnr_4s_list))
    print("SSIM:", np.mean(ssim_c_list), np.mean(ssim_1s_list), np.mean(ssim_2s_list), np.mean(ssim_3s_list), np.mean(ssim_4s_list))
